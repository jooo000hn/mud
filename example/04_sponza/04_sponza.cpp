...
void ex_04_sponza(Shell& app, Widget& parent, Dockbar& dockbar)
{
	...
	static float altitude = float(M_PI) / 2.f - 0.01f;
	...
	if(Widget* dock = ui::dockitem(dockbar, "Game", carray<uint16_t, 1>{ 1U }))
	{
		...
		ui::label(sheet, "Sun :");
		ui::slider_field<float>(sheet, "Azimuth", { azimuth, { 0.f, float(M_PI), 0.01f } });
		ui::slider_field<float>(sheet, "Altitude", { altitude, { 0.f, float(M_PI) / 2.f, 0.01f } });
	}
}
